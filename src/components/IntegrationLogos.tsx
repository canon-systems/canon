...
function GongIcon({ size = 24 }: { size?: number }) {
  return (
    <svg width={size} height={size} viewBox="-0.2 0 38.5 42" role="img" aria-label="Gong">
      <title>Gong</title>
...
const IMAGE_LOGOS: Record<string, { alt: string }> = {
  slack: { alt: 'Slack' },
  granola: { alt: 'Granola' },
...
export function IntegrationLogos({ size = 24, provider, logoUrl, color }: IntegrationLogosProps) {
  const imageLogo = IMAGE_LOGOS[provider];
  if (imageLogo) {
    return (
      // Nango logo URLs are runtime data, so use a normal image element.
      // eslint-disable-next-line @next/next/no-img-element
      <img
        src={logoUrl ?? nangoLogoPath(provider)}
        alt={imageLogo.alt}
        width={size}
        height={size}
  ...
  return (
    <span aria-hidden="true" style={{ color, fontSize: size * 0.58, fontWeight: 700, lineHeight: 1 }}>
