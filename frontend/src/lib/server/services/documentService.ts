...
export async function getDocument(
	supabase: SupabaseClient,
	documentId: string
): Promise<{ id: string; title: string; content: string; repo_id: string; kb_id: string | null; kb_provider: string | null } | null> {
	const { data, error } = await supabase
		.from('documents')
		.select('id, title, content, repo_id, kb_id, kb_provider')
		.eq('id', documentId)
		.single();
