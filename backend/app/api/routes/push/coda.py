...
@router.post("/coda", response_model=PushResponse)
async def push_to_coda(
    request: PushRequest,
    user: dict = Depends(get_current_user),
    supabase: Client = Depends(get_supabase)
):
    """
    Publish the finalized documentation to Coda.
    Creates a new doc or updates an existing one.
    """
    try:
        # Get user's connection
        connection = supabase.table('oauth_connections').select('connection_id').eq(
            'user_id', user['id']
        ).eq('provider', 'coda').eq('status', 'active').single().execute()
        
        if not connection.data:
            raise ValueError("No active Coda connection found for user")
        
        connection_id = connection.data['connection_id']
        
        # Get workspace provider
        provider = get_workspace_provider('coda', supabase)
        if not provider:
            raise ValueError("Coda provider not available")
        ...
        if not result:
            raise Exception("Failed to push content to Coda")
