...
@router.post("/notion", response_model=PushResponse)
async def push_to_notion(
    ...
    """
    Publish the finalized documentation to Notion.
    ...
    try:
        ...
        connection = supabase.table('oauth_connections').select('connection_id').eq(
            'user_id', user['id']
        ).eq('provider', 'notion').eq('status', 'active').single().execute()
        
        if not connection.data:
            raise ValueError("No active Notion connection found for user")
        
        connection_id = connection.data['connection_id']
        
        # Get workspace provider
        provider = get_workspace_provider('notion', supabase)
        if not provider:
            raise ValueError("Notion provider not available")
